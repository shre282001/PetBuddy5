...
class _dogCardState extends State<dogCard> {
  ...
  void getData() async {
    var snapshot =
    await FirebaseFirestore.instance.collection('Pets').where('type', isEqualTo: 'Dog').get();
...
class ProductCard extends StatelessWidget {
  ...
  Future<List<Widget>> listAllImages() async {
    List<Reference> imageRefs = [];
    // Storage reference
    Reference storageRef = FirebaseStorage.instance.ref('petImages');
    ...
    for (final Reference item in result.items) {
      if (item.name.endsWith('.jpg') || item.name.endsWith('.png')) {
        // Filter images by file extension (adjust as needed)
        imageRefs.add(item);
      }
    }
    ...
    for (Reference imageRef in imageRefs) {
      ...
      // Add the 'imageWidget' to your widget tree to display the image
